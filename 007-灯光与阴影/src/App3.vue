...
// 距离衰减大于0时才有效
correctFolder.add(pointLight, "decay").min(0).max(5).step(0.01).name("正确距离衰减");
// 光功率 http://localhost:8080/docs/#api/zh/lights/PointLight.power
correctFolder.add(pointLight, "power").min(0).max(pointLight.intensity * Math.PI * 10).step(0.01).name("光功率");
